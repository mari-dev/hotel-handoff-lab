...
This is a standalone, autonomous project. Do not reference, copy from, or edit
any other local repository from this project.
Use synthetic data only. Never commit credentials or real hotel records.
Keep this repository private until the user explicitly authorizes publication.
Record reused material and its license before incorporating any source code.
Do not relabel GPL or unclear-license source as MIT/Apache.
Separate model extraction, evidence checks, and human-confirmed actions.
Do not claim model integration works from offline tests alone.
Keep dated progress notes here. Mark agent-written prose as awaiting review.
...
> **Status: not yet manually reviewed.** Setup context added by an agent; awaiting human review.

Profile: `hotel-handoff-lab`. Selected Region: `eu-north-1`. New AWS experience, paid project with a user-configured monthly USD 20 spend limit. Preserve the user’s standing instructions to work autonomously and provide short, practical guidance; do not repeat previously answered setup questions.

The guidance below is reproduced from https://raw.githubusercontent.com/aws/agent-toolkit-for-aws/refs/heads/main/rules/aws-starter-rules.md as requested by the official setup workflow; existing project and user instructions take precedence.
