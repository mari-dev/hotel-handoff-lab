...
## Inspiration
This comes from lived experience, not a hypothetical: working front-of-house at a small independently-run hotel, covering both restaurant/bar service and reception in the same shift. Staffing is mostly part-time and minijob workers with high turnover and little formal onboarding, so coordination between shifts is inconsistent by default, not by anyone's fault. The owner-manager handles the business side personally but has minimal software fluency and will not use anything beyond the simplest possible click.
...
**Persona:** front-of-house staff at a small independent hotel who split their time between restaurant/bar service and the reception desk, working alongside other part-time and minijob colleagues who rotate frequently; the owner-manager oversees invoicing personally but has very limited comfort with software.
...
- **Duplicate or dropped work.** With high turnover and minimal handover process, two different part-time staff can each start preparing the same guest's invoice without knowing about the other, or a request can quietly fall through because no one is clearly its owner. Checking existing requests by guest name before creating a new one directly targets this.
...
## What it does
A Strands agent reads a synthetic guest message, extracts an explicit invoice request and its literal details, and checks existing requests by guest name. Every extracted field is shown with source text. Missing details stay empty. Staff review the proposal and explicitly confirm creation.
