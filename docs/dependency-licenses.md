# Dependency license inventory — 2026-09-13

These are unmodified dependencies from the locked Python environment. Original license files remain inside their distributions and in the judge ZIP. No dependency is relicensed as project code. Metadata alone is not a legal audit.

| Distribution | Version | Declared license |
|---|---|---|
| annotated-types | 0.8.0 | MIT |
| anyio | 4.15.1 | MIT |
| attrs | 26.1.0 | MIT |
| awscrt | 0.36.0 | Apache-2.0 |
| boto3 | 1.43.93 | Apache-2.0 |
| botocore | 1.43.93 | Apache-2.0 |
| certifi | 2026.7.22 | Mozilla Public License 2.0 (MPL 2.0) |
| cffi | 2.1.1 | MIT-0 |
| click | 8.5.0 | BSD-3-Clause |
| cryptography | 50.0.1 | Apache-2.0 OR BSD-3-Clause |
| docstring_parser | 0.18.0 | MIT License |
| h11 | 0.16.0 | MIT License |
| httpcore | 1.0.9 | BSD-3-Clause |
| httpcore2 | 2.12.0 | BSD-3-Clause |
| httpx | 0.28.1 | BSD License |
| httpx2 | 2.12.0 | BSD-3-Clause |
| idna | 3.19 | BSD-3-Clause |
| jmespath | 1.1.0 | MIT License |
| jsonschema | 4.26.0 | MIT |
| jsonschema-specifications | 2025.9.1 | MIT |
| mcp | 2.1.1 | MIT License |
| mcp-types | 2.1.1 | MIT License |
| opentelemetry-api | 1.44.0 | Apache-2.0 |
| opentelemetry-instrumentation | 0.65b0 | Apache-2.0 |
| opentelemetry-instrumentation-threading | 0.65b0 | Apache-2.0 |
| opentelemetry-sdk | 1.44.0 | Apache-2.0 |
| opentelemetry-semantic-conventions | 0.65b0 | Apache-2.0 |
| packaging | 26.3 | Apache-2.0 OR BSD-2-Clause |
| pycparser | 3.0 | BSD-3-Clause |
| pydantic | 2.13.5 | MIT |
| pydantic_core | 2.46.5 | MIT |
| PyJWT | 2.14.0 | MIT |
| pypdf | 6.18.1 | BSD-3-Clause |
| python-dateutil | 2.9.0.post0 | BSD License, Apache Software License |
| python-multipart | 0.0.32 | Apache-2.0 |
| PyYAML | 6.0.3 | MIT License |
| referencing | 0.37.0 | MIT |
| rpds-py | 2026.6.3 | MIT |
| s3transfer | 0.19.2 | Apache Software License |
| six | 1.17.0 | MIT License |
| sse-starlette | 3.4.11 | BSD-3-Clause |
| starlette | 1.6.0 | BSD-3-Clause |
| strands-agents | 1.55.1 | Apache Software License |
| truststore | 0.10.4 | MIT |
| typing-inspection | 0.4.4 | MIT |
| typing_extensions | 4.16.0 | PSF-2.0 |
| urllib3 | 2.7.0 | MIT |
| uvicorn | 0.52.4 | BSD-3-Clause |
| watchdog | 6.0.0 | Apache Software License |
| wrapt | 2.4.1 | BSD-2-Clause |

| MarkupSafe | 3.0.3 | BSD-3-Clause |
| Werkzeug | 3.1.8 | BSD-3-Clause |

Vendored browser dependency: i18next 26.4.2, MIT; original notice retained in web/vendor/i18next.LICENSE. Sources and integrity verification are recorded in docs/reuse.md.
